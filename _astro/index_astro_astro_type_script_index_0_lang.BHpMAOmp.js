...
/* ---- Status badges (top-right of card image) ---- */
function statusBadge(p) {
  if (isPast(p)) {
    return `<span style="position:absolute;top:14px;right:14px;display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:12px;font-family:ui-monospace,monospace;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;font-weight:600;background:rgba(0,0,0,.72);color:rgba(255,255,255,.85);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);">
  ...
  if (isSoldOut(p)) {
    return `<span style="position:absolute;top:14px;right:14px;display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:12px;font-family:ui-monospace,monospace;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;font-weight:700;background:#0a0a0a;color:#fff;box-shadow:0 4px 14px rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.06);">
...
function leftBadges(p) {
  ...
  if (toNumber(p.promo_price) > 0 && toNumber(p.price) > toNumber(p.promo_price)) {
    out.push(`<span style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;border-radius:12px;font-family:ui-monospace,monospace;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;font-weight:700;background:linear-gradient(135deg,rgb(var(--brand)),#dc2626);color:#fff;box-shadow:0 4px 14px rgba(220,38,38,.35);">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2 L3 9 L12 9 L8 22 L21 11 L13 11 z"/></svg>
                Promo
              </span>`);
  } else if (p.badge) {
    out.push(`<span style="display:inline-flex;align-items:center;padding:6px 12px;border-radius:12px;font-family:ui-monospace,monospace;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;font-weight:600;background:rgb(var(--surface));color:rgb(var(--ink-1,24 24 27));box-shadow:0 2px 8px rgba(0,0,0,.08);">${esc(p.badge)}</span>`);
  }
  return out.length
...
function priceArea(p) {
  if (isPast(p)) {
    return `<p style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;font-size:14px;color:rgb(var(--ink-3));margin:0;">Trip selesai</p>`;
  }
  if (isSoldOut(p)) {
    return `<p style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;font-size:14px;color:rgb(var(--ink-3));margin:0;">Sold out</p>`;
  }
...
function renderCards(list) {
  ...
  root.innerHTML = list
    .map((p) => {
      ...
      const ctaLabel = past ? "Lihat Cerita" : soldOut ? "Lihat Detail" : "Lihat Perjalanan";
      const img = p.hero_img
        ? `<img src="${esc(p.hero_img)}" alt="${esc(p.title)}" loading="lazy"
             class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
             style="${inactive ? 'filter:grayscale(1) brightness(.92);opacity:.8;' : ""}">`
        : "";

      // Card wrapper styles for inactive
      const cardStyle = inactive
        ? 'opacity:.75;'
        : '';
      // Subtle overlay for past/sold-out image area
      const overlay = inactive
        ? `<div style="position:absolute;inset:0;background:linear-gradient(180deg,transparent 40%,rgba(0,0,0,.35) 100%);pointer-events:none;z-index:1;"></div>`
        : '';
        ...
        <article class="card card-hoverable overflow-hidden flex flex-col group"
          ...
          <a href="${link}" class="block">
            <div class="relative aspect-[4/3] overflow-hidden bg-surface-muted">
              ${img}${overlay}${leftBadges(p)}${statusBadge(p)}
            </div>
          </a>
          <div class="p-4 sm:p-5 flex-1 flex flex-col">
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px;">${chips(p)}${seatsLine(p)}</div>
            <h3 class="font-display font-bold text-base sm:text-lg leading-tight tracking-tight mb-1.5 text-balance" ${inactive ? 'style="color:rgb(var(--ink-3));"' : ''}>
...
const init = async () => {
  // Hide the duplicate small stat strip in hero
