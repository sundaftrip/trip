...
export default function StickyWhatsApp({ phone }: { phone: string }) {
  ...
  return (
    <a
      ...
      className="fixed inset-x-3 bottom-3 z-50 flex min-h-14 items-center gap-3 rounded-xl px-3 py-2 text-white shadow-lg shadow-black/25 transition active:scale-[0.99] sm:inset-x-auto sm:bottom-6 sm:right-6 sm:h-14 sm:min-h-0 sm:rounded-full sm:py-0 sm:pl-3 sm:pr-4 sm:hover:scale-105"
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col leading-tight sm:hidden">
        <span className="text-[11px] font-medium opacity-85">Siap join trip?</span>
        <span className="truncate text-sm font-bold">Chat Sundaf Trip</span>
      </span>
      <span className="rounded-lg bg-white/20 px-3 py-2 text-xs font-bold sm:hidden">Join via WA</span>
