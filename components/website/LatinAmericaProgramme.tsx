  ...
  return (
    <div className={styles.page} id="main-content">
        ...
        <div className={styles.shell}>
          ...
          <div className={styles.heroGrid}>
            <div>
              ...
              <ul className={styles.tripFacts}><li><CalendarDays size={17} aria-hidden="true" /> {detail.duration}</li><li><Users size={17} aria-hidden="true" /> Grup 10, 15, atau 20 peserta</li><li><Plane size={17} aria-hidden="true" /> Tiket pesawat PP dari Jakarta</li></ul>
              <div className={styles.heroPrice}><span>Estimasi mulai</span><strong>{rupiahMillions(detail.price.from)}</strong><small>per orang · untuk 20 peserta</small></div>
      ...
      <section className={styles.planning} id="layanan" aria-labelledby="inclusions-heading"><div className={`${styles.shell} ${styles.inclusionsGrid}`}>
        <div><p className={styles.eyebrow}>CAKUPAN PAKET</p><h2 id="inclusions-heading">Sudah termasuk</h2><ul className={styles.includedList}>{detail.included.map((item) => <li key={item}><Check size={17} aria-hidden="true" /><span>{item}</span></li>)}</ul></div>
      ...
      <section className={`${styles.section} ${styles.shell}`} aria-labelledby="faq-heading"><h2 id="faq-heading">Sebelum menentukan tanggal</h2><div className={styles.faq}>
        <StableDetails><summary>Apakah harga sudah termasuk tiket dari Jakarta?</summary><p>Ya, harga paket mencakup tiket internasional PP dari Jakarta, penerbangan domestik atau regional sesuai itinerary, dan pendampingan tour leader dari Indonesia. Jadwal dan maskapai dikonfirmasi sebelum pemesanan.</p></StableDetails>
        <StableDetails><summary>Bisa berangkat dengan 10 atau 15 peserta?</summary><p>Bisa. Pilih jumlah peserta pada bagian harga untuk melihat estimasinya. Pilihan 15 peserta mencakup 7 kamar berdua dan 1 kamar sendiri untuk peserta. Untuk jumlah lainnya, hubungi tim Sundaf.</p></StableDetails>
        <StableDetails><summary>Bisa meminta kamar sendiri atau hotel 4 bintang?</summary><p>Upgrade hotel tersedia pada pilihan tambahan dan mengikuti jumlah peserta yang dipilih. Kamar sendiri di luar susunan kamar paket dapat diminta melalui penawaran terpisah.</p></StableDetails>
