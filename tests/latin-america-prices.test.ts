...
test("every public group size has a complete retail price and option schedule", () => {
  for (const [id, detail] of Object.entries(LATIN_AMERICA_PACKAGES)) {
    ...
    assert.match(packageGroup(price, 15).roomNote, /7 kamar berdua \+ 1 kamar sendiri/);
  }
});
...
test("group changes reprice selected options without duplicating charges", () => {
  const price: PackagePrice = {
    ...
    groups: [
      { groupSize: 10, from: 160_000, optionPrices: { hotel: 35_000, rail: 15_000 }, roomNote: "5 twin" },
      { groupSize: 15, from: 130_000, optionPrices: { hotel: 25_000, rail: 15_000 }, roomNote: "7 twin + 1 single" },
      { groupSize: 20, from: 100_000, optionPrices: { hotel: 20_000, rail: 15_000 }, roomNote: "10 twin" },
    ],
  };
