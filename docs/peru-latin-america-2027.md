...
The price selector first chooses a package with flights or land tour only, then 10, 15 or 20 paying guests. It updates the retail estimate, optional hotel upgrade, room arrangement and WhatsApp enquiry together. Previously selected options stay selected when group size or package mode changes and use the new group's prices. Land-only excludes every paying participant flight, including required domestic or regional sectors; both modes include one Indonesian tour leader. The lowest advertised price refers to 20 guests. The 15-guest arrangement uses seven shared rooms plus one room shared by the 15th guest and the tour leader. No participant single room is included by default; a requested single room requires a separate offer.
...
- `scripts/latin-america-brochure/generate_brochure.py`: six-page PDF from the same public data, including both package modes at all three group sizes, room arrangements and optional hotel rates.

`price.from` and `price.options[].amount` describe the default group. `price.groups[]` supplies `from` for the package with flights, `landOnlyFrom` for ground services without participant flights, and every option amount for each selectable group. `landTour` defines local duration, meeting/end points and flight sectors the land-only guest arranges separately. Peru ground services start at LIM and end at CUZ; four-country services start/end at GRU. `included` lists only services common to both modes; `flightInclusions` describes the package with flights. Tests ensure the default values agree with its group entry, all options have prices, and unsupported group sizes cannot produce a misleading total.
...
Render all six pages for visual inspection. Preserve bundled font licences, photo provenance and on-page attribution. The generator rejects procurement fields and research terminology in the public package data.
...
- Check native radio selection and keyboard navigation for both package modes at all three group sizes, including retained options, recomputed totals, room notes, flight exclusions and the WhatsApp message.
...
The static catalogue PDF is covered by the service worker network-only rule and no-store response headers. Activation removes older cached versions of that PDF, including query variants, while retaining unrelated assets. PDF failures cannot become the offline HTML fallback. The existing worker harness covers these cases.
