...
const EXACT_LINES = new Map<string, string>([
  ...
  ["Sarapan at Hotel", "Sarapan di hotel"],
  ...
  ["Optional, subject to availability.", "Opsional, mengikuti ketersediaan."],
  ["Private Land Tour", "Land Tour Privat"],
]);
...
const REPLACEMENTS: Array<[RegExp, string]> = [
  ...
  [/Central Vietnam/gi, "Vietnam Tengah"],
  [/Southern Vietnam/gi, "Vietnam Selatan"],
  [/South Vietnam/gi, "Vietnam Selatan"],
  [/South to North/gi, "Selatan ke Utara"],
  [/From North to South/gi, "dari Utara ke Selatan"],
  [/\bHighlights:/gi, "Sorotan:"],
  [/\bPrivate Land Tour Only\b/gi, "land tour privat"],
  [/\bPrivate Land Tour\b/gi, "Land Tour Privat"],
  [/\bSpa treatment\b/gi, "Spa"],
  [/\bmassage\b/gi, "pijat"],
  ...
  [/\bwith\b/gi, "dengan"],
  [/\bin Hanoi\b/gi, "di Hanoi"],
  [/\btour leader\b/gi, "pemimpin tur"],
  [/\bincluded\b/gi, "termasuk"],
  [/\bat night\b/gi, "malam hari"],
  [/\badventure\b/gi, "petualangan"],
  [/\bmins\b/gi, "menit"],
  [/\b(\d+)-hour\b/gi, "$1 jam"],
  [/Halong Bay Day Cruise/gi, "Pelayaran Harian Teluk Halong"],
  [/Day Cruise/gi, "Pelayaran Harian"],
  ...
  [/\bMeals\b/gi, "Makan"],
  [/\bHave Makan siang di a nice local restaurant\b/gi, "Makan siang di restoran lokal"],
  [/\bEnjoy Makan malam in a nice local restaurant\b/gi, "Makan malam di restoran lokal"],
  [/\bMakan malam in a nice local restaurant\b/gi, "Makan malam di restoran lokal"],
  [/\bMakan siang di a nice local restaurant\b/gi, "Makan siang di restoran lokal"],
  [/\bBegin your exploration of Hanoi dengan a relaxing one-hour cyclo ride through the charming Hanoi Old Quarter\b/gi, "Mulai eksplorasi Hanoi dengan cyclo ride santai selama satu jam melalui Hanoi Old Quarter"],
  [/\bAs you travel at a leisurely pace through its narrow, bustling streets, you'll gain a unique perspective on local life dan the rich heritage of this historic neighborhood\b/gi, "Saat melewati jalan-jalan sempit yang ramai, Anda dapat melihat kehidupan lokal dan warisan budaya kawasan bersejarah ini dari dekat"],
  [/\bAt approximately 5:00 PM, Kembali ke hotel Anda, bringing an end ke a fascinating day of cultural discovery in Vietnam's capital city\b/gi, "Sekitar pukul 17.00, kembali ke hotel dan menutup hari eksplorasi budaya di ibu kota Vietnam"],
  [/\bAt 05:00 PM, transfer from Sapa ke Hanoi by shared limousine\b/gi, "Pukul 17.00, transfer dari Sapa ke Hanoi dengan shared limousine"],
  [/\bKedatangan di Hanoi is expected in the evening\b/gi, "Tiba di Hanoi diperkirakan pada malam hari"],
  [/\bToday, you are free at your leisure until being transferred by private van \(tanpa pemandu\) menuju bandara untuk penerbangan pulang\b/gi, "Hari ini waktu bebas sampai dijemput dengan van privat (tanpa pemandu) menuju bandara untuk penerbangan pulang"],
  [/\bBreakfast\b/gi, "Sarapan"],
  [/\bSarapan at Hotel\b/gi, "Sarapan di hotel"],
  ...
  [/\bNo meal\b/gi, "belum termasuk makan"],
...
export function localizePdfTour<T extends LocalizablePdfTour>(tour: T): T {
  return {
    ...
    description: localizePdfText(tour.description),
